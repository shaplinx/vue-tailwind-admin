  ...
  <div class="grid grid-cols-1 gap-5">
    <dv-navbar class="bg-base-100 rounded-box">
      <div class="flex w-full justify-end gap-2 items-center">
        ...
        />
        <dv-button
          @click="back({ name: 'SuratSehatIndex' })"
          variant="secondary"
          size="sm"
        ...
        <dv-button
          @click="
            edit({ name: 'SuratSehatForm', params: { id: route.params.id } })
          "
          variant="warning"
          size="sm"
      ...
      <dv-card-body>
        <div id="print-area">
          ...
          <h2 class="text-center mb-4">
            {{ t("suratSehat.ref_number") }} {{ model.ref_number }}
          </h2>
          ...
          <table class="mb-2">
            <tr>
              <td>
                {{ t("pemeriksaan.form.TB") }}/{{ t("pemeriksaan.form.BB") }}
              </td>
              <td>:</td>
              <td>
                {{ model.pertemuan?.pemeriksaan?.TB ?? "-" }}
                {{ t("formkit.ttv.tb") }} /
                {{ model.pertemuan?.pemeriksaan?.BB ?? "-" }}
                {{ t("formkit.ttv.bb") }}
              </td>
            </tr>
            <tr>
              ...
              <td>:</td>
              <td>
                {{ model.pertemuan?.pemeriksaan?.tekanan_darah }}
                {{ t("formkit.ttv.bp") }}
              </td>
            </tr>
            <tr>
          ...
          <p class="mb-6">
            {{ t("suratSehat.closing") }}
            {{ model.keperluan ?? t("suratSehat.default_keperluan") }}.
          </p>
